...
__global__ void CurrentModulator(
    int num,
    %(type)s dt,
    %(type)s *g_I,
    %(type)s *A,
    ...
    %(type)s *out)
{
    ...
    for( int i=tid; i<num; i+=tot_threads ){
        internal_var[i] = A[i] * g_I[i] + shift[i];
        out[i] = internal_var[i];
    }
    return;
}
"""
class CurrentModulator(BaseSynapseModel):
    accesses = ['I']
    updates = ['I']
    def __init__( self, params_dict, access_buffers, dt,
        ...
        self.update = self.get_gpu_kernel(params_dict['A'].dtype)
    ...
    def run_step(self, update_pointers, st = None):
        ...
        self.update.prepared_async_call(
            ...
            self.params_dict['A'].gpudata,\
            self.params_dict['shift'].gpudata,\
    ...
    def get_gpu_kernel(self, dtype=np.double):
        ...
        func = mod.get_function("CurrentModulator")
