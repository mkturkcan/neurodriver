    ...
    %(type)s *out)
{
    ...
    for( int i=tid; i<num; i+=tot_threads ){
        out[i] = beta[i] * powf(max(0.0, g_I[i]), n[i]) / (powf(K[i], n[i]) + powf(g_I[i], n[i]));
    }
    return;
}
"""
class Activator(BaseSynapseModel):
    accesses = ['I']
    updates = ['I']
    def __init__( self, params_dict, access_buffers, dt,
        ...
        self.debug = debug
        self.dt = dt
        self.num = params_dict['beta'].size
        self.LPU_id = LPU_id
        ...
        self.update = self.get_gpu_kernel(params_dict['beta'].dtype)
    ...
    def run_step(self, update_pointers, st = None):
        ...
        self.update.prepared_async_call(
            ...
            self.params_dict['beta'].gpudata,\
            self.params_dict['K'].gpudata,\
            self.params_dict['n'].gpudata,\
            update_pointers['I'])
    ...
    def get_gpu_kernel(self, dtype=np.double):
        ...
        func = mod.get_function("activator")
        func.prepare('idPPPPP')
        return func
